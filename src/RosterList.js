        ...
        if(this.state.studentsLoaded){
            return(
                <div style={{borderStyle: "dashed", borderWidth: 2, margin: 10}}>
                    {studentArray.map((student) => <StudentBox  student={student}/>)}
            ...
            return(
                <div style={{borderStyle: "dashed", borderWidth: 2, margin: 10}}>
                    {studentArray.map((student) => <StudentBox key={student} student={student}/>)}
                </div>
            );
        }
    }
    ...
    render() {
        return(
            <div style={{borderStyle: "solid", borderWidth: 1, margin: 4, marginRight: 8, marginLeft: 8}}>
                ...
                <Button variant="outlined" color="error" style={{alignContent: "right", marginBottom: 10}} onClick={console.log("lol")}>Remove from Class</Button>
            </div>
        );
    }
}
...
class AddNewStudent extends React.Component {
    ...
    render() {
        ...
        console.log(curStudentArray); 
        console.log(this.arrayIncludesValue(curStudentArray, {
            "GPA": 3.3,
            "fname": "Jason",
            "lname": "Smith"
        }));
